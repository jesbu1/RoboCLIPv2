...
class MetaworldSparse(Env):
        ...
        # load xclip model
            ...
            if args.train_orcale:
                if not args.target_gif_path:
                    ...
                    frames = adjust_frames_xclip(frames, target_frame_count = args.frame_length, processor=self.processor).cuda()
    ...
    def warm_up_run(self):
        self.env.reset()
        images = []
        # random int between frame_length and 128
        frame_num = random.randint(self.args.frame_length, 128)
...
def main():
    ...
    experiment_name = "xclip_k600_model_sac_baseline_" + args.env_id + "_" + args.algo + "_" + str(args.seed)
    if args.train_orcale:
    ...
    if args.wandb:
        run = wandb.init(
            ...
            group="xclip_sac_baselineFIX_" + args.env_id,
            config=args,
