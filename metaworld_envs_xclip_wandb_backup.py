...
def get_args():
    ...
    parser.add_argument('--text-string', type=str, default='closing door')
    parser.add_argument('--dir-add', type=str, default='')
    parser.add_argument('--env-id', type=str, default='door-close-v2-goal-hidden')
    parser.add_argument('--env-type', type=str, default='sparse_learnt')
    ...
    parser.add_argument('--video-freq', type=int, default=2560)
...
#         # Capture a video every 1000 steps
#         if self.n_calls % 1000 == 0:
#             self.episode_id += 1
#             video = self.env.render(mode='rgb_array')
#             video_path = f"video_{self.episode_id}.mp4"
#             wandb.log({f"video_{self.episode_id}": wandb.Video(video, fps=4, format="mp4")})
#         return True
...
def main():
    ...
    text_string_value = args.text_string
    env_id_value = args.env_id

    # wandb.log({"text_string": text_string_value, "env_id": env_id_value})
    column1 = ["text_string"]
    ...
    # table2 = wandb.Table(columns=1)
    # table2.add_data([args.env_id])
    # # table.add_data(args.env_id)
    # wandb.log({"env_id": table2})
