...
h5_file = h5py.File("/scr/jzhang96/metaworld_xclip_embeddings.h5", "w")

folder_prefix = "/scr/jzhang96/metaworld_generation_true_video/videos_"
for task_idx in tqdm(task_id.keys()):
    task_name = task_id[task_idx]
    task_annotation = task_ann[task_name]
    task_folder = folder_prefix + task_name

    if check_folder(task_folder, num_videos=30):
