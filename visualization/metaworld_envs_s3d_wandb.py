...
import torch as th
from s3dg import S3D
from Dataload import TextEmbedding
import numpy as np
from PIL import Image, ImageSequence
import cv2
import gif2numpy
...
class MetaworldSparse(Env):
    def __init__(self, env_id, text_string=None, time=False, video_path=None, rank=0, human=True):
        ...
        if text_string:
            ...
            text_emb = text_embedding._words_to_ids([text_string])
            text_output = self.net.text_module(text_emb)
        ...
        if self.time:
            ...
            if t - 1 == 0:
                if self.counter_total % 1280 == 0:
                    path = "/scr/yusenluo/RoboCLIP/visualization/metaworld_gifs/s3d_draw_close/"+"door_opening_"+str(self.rank)
...
def main():
    ...
    experiment_name = "s3d-wandb_" + args.env_id + "_" + str(args.seed)
    ...
    log_dir = f"/scr/yusenluo/RoboCLIP/visualization/s3d_logs"  # change to your log directory
