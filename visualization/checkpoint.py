...
from Transformation_Matrix import  eval_M
...
if __name__ == "__main__":
    ...
    for sample_size in sample_sizes:
        ...
        No PCA AFTER MLP
        """
        for check_point1 in check_points:
            mlp_model_path = f"saved_model/mlp_model/checkpoint/checkpoint_{check_point1}__{1}_{sample_size}.pth"
            ...
            plot_embeddings(
                ...
                train_text_embeddings_normalized.numpy(),
                train_mappings,
                f"plots/taskB/checkpoint{check_point1}/{plot_dir_name}/mlp",
                f"pca_plot_mlp2D_{check_point1}_{1}_{sample_size}.png",
                False,
            )
        ...
        for variance_threshold in variance_thresholds:
            ...
            pca_video_model_path = (
                f"saved_model/pca_model/pca_model_video_{variance_threshold}_{sample_size}.pkl"
            )
            video_pca = joblib.load(pca_video_model_path)
            pca_text_model_path = (
                f"saved_model/pca_model/pca_model_text_{variance_threshold}_{sample_size}.pkl"
            )
            ...
            for check_point in check_points:
                ...
                """
                print(
                    f"Results with checkpoint {check_point} variance_threshold {variance_threshold} and {sample_size}:"
                )
                ...
                mlp_model_path = (
                    f"saved_model/mlp_model/checkpoint/checkpoint_{check_point}__{variance_threshold}_{sample_size}.pth"
                )
                adjusted_video_embeddings = mlp_eval(
                ...
                print(
                    f"Result after MLP_{check_point}_{variance_threshold}_{sample_size} in {adjusted_video_embeddings.shape[1]}D space"
                )
                check_pairs(
                    adjusted_video_embeddings.cpu().numpy(),
                    text_embeddings_pca,
                ...
                plot_embeddings(
                    ...
                    text_embeddings_pca,
                    train_mappings,
                    f"plots/taskB/checkpoint{check_point}/{plot_dir_name}/mlp",
                    f"pca_plot_mlp_{check_point}_{variance_threshold}_{sample_size}.png",
                    False,
                )
