...
def compute_pearson_correlation_from_sequences(
    ...
    for i, seq in enumerate(all_seqs):
        ...
        # 逐个 log 到 wandb
        wandb.log({f"{set_type}_correlation/{env_name}_pearson": corr_val})
    ...
    wandb.log({f"{set_type}_correlation/average_pearson": avg_corr})
...
if __name__ == "__main__":
    ...
    conf_mat, all_seqs, tasks, text_list = generate_gemini_data(
        h5_path="/scr/yusenluo/RoboCLIP/visualization/decoder_only/all_fail_videos.h5",
    ...
    wandb.init(project="roboclip-v2", name=f"eval_GVL_all_fail")
