...
def plot_progress_class(h5_file, set, self_attention_model, args):
    ...
    for env in eval_envs:
        env_name = env + "_text"
        env_name = "text_annotations/" + env_name
    ...
    print("text_embeddings shape", text_embeddings.shape) # (num_envs, 1024)
    ...
    for i  in tqdm(range(len(eval_envs))):
        ...
        print(f"{env} : {predicted_classes}")
        gt_array = np.linspace(1, len(predicted_classes), len(predicted_classes), dtype=np.float32)
        MSE_gt_array = np.linspace(0, 1, len(predicted_classes), dtype=np.float32)
        mse_val = mean_squared_error(MSE_gt_array, predicted_classes)
        final_mse_val = mean_squared_error([MSE_gt_array[-1]], [predicted_classes[-1]])
        mse_list.append(mse_val)
        final_mse_list.append(final_mse_val)
        spear_corr, _ = spearmanr(predicted_classes, gt_array)
        pearson_corr, _ = pearsonr(predicted_classes, gt_array)
        spearman_correlations.append(spear_corr)
        pearson_correlations.append(pearson_corr)
        wandb.log({f"{set}_pearson_correlation/{env}_pearson": pearson_corr})
        wandb.log({f"{set}_pearson_correlation/{env}_pearson": spear_corr})
    ...
    wandb.log({f"{set}_spearman_corrleation/average_spearman": float(np.mean(spearman_correlations))})
    wandb.log({f"{set}_pearson_corrleation/average_pearson": float(np.mean(pearson_correlations))})
    # Log 平均值
    wandb.log({f"{set}_mse/average_overall_mse": avg_mse})
    wandb.log({f"{set}_mse/average_final_mse": avg_final_mse})
