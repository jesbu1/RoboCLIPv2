...
def padding_video(video_frames, max_length):
    ...
    if video_length < max_length:
        ...
        assert video_frames.shape[0] == max_length
...
def plot_matrix_as_image(matrix, names, set, text):
    ...
    wandb.log({f"confusion_matrix/{set}_confusion_matrix_S3D": wandb.Image(fig)})
    plt.savefig(f"confusion_matrix_{set}_S3D.pdf", bbox_inches="tight")
    plt.close(fig)  # Close the figure to free memory


def generate_s3d_data(
    h5_path: str,
    json_path: str,
    set_type: str,
    device: str = "cuda",
    cache_path="s3d_cache.pkl"
):
    """
        ...
        for i, env_video_name in enumerate(tasks):
            ...
            for j, text_str in enumerate(text_list):
                ...
                if cache_key_full in cache_dict:
                    ...
                    #       并返回 [0..1] reward
                    imgs = padding_video(frames_data, 32)
                    imgs = imgs.permute(3, 0, 1, 2).unsqueeze(0).to(device).float()
                    print(f"🎬 {env_video_name} ({text_str}) => {imgs.shape}")
                    text_embeddings = net.text_module([text_str])["text_embedding"].to(device).float()
                    text_embeddings = normalize_embeddings(text_embeddings)
                    print(f"📝 {env_video_name} ({text_str}) => text_embed: {text_embeddings.shape}")
                    video_embeddings = net(imgs)["video_embedding"].to(device).float()
                    video_embeddings = normalize_embeddings(video_embeddings)
                    print(f"🎬 {env_video_name} ({text_str}) => video_embed: {video_embeddings.shape}")
                    video_reward = torch.matmul(video_embeddings, text_embeddings.t())[0].detach().cpu().numpy()
                    ...
                    if total_frames < 1:
                        ...
                        for partial_count in range(1, total_frames + 1):
                            ...
                            if cache_key_partial in cache_dict:
                                ...
                                sub_frames = frames_data[:partial_count]
                                #  print(f"🎬 {env_video_name} ({text_str}) => {sub_frames.shape}")
                                imgs = padding_video(sub_frames, 32)
                                imgs = imgs.permute(3, 0, 1, 2).unsqueeze(0).to(device).float()
                                video_embeddings = net(imgs)["video_embedding"].to(device).float()
                                mat = torch.matmul(video_embeddings, text_embeddings.t())
                                # print(f"🎬 {partial_count}) => {mat.shape}")
                                sub_reward = (mat[0][0].detach().cpu().numpy().squeeze()).item()
...
def compute_pearson_correlation_from_sequences(
    ...
    for i, seq in enumerate(all_seqs):
        ...
        if len(seq) < 2:
            ...
            # 将 [0..100] 转为 [0..1]
            seq = [float(x) if isinstance(x, (list, np.ndarray)) else x for x in seq]
...
if __name__ == "__main__":
    ...
    net.eval()
    # ============ 1) 生成数据 ============
    confusion_matrix, predicted_sequences, tasks, text_list = generate_s3d_data(
    ...
    wandb.init(project="roboclip-v2", name=f"eval_S3D_rank")
    ...
    confusion_matrix_close_success, _, _, _ = generate_s3d_data(
        h5_path="/scr/yusenluo/RoboCLIP/visualization/close_succ_videos.h5",
        json_path="new_task_v2.json",
        set_type="eval",
        device="cuda",
        cache_path="s3d_cache_close_success.pkl"
    )

    confusion_matrix_all_fail, _, _, _ = generate_s3d_data(
        h5_path="/scr/yusenluo/RoboCLIP/visualization/all_fail_videos.h5",
        json_path="new_task_v2.json",
        set_type="eval",
        device="cuda",
        cache_path="s3d_cache_all_fail.pkl"
    )
