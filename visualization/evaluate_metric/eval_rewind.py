...
from eval_utils_decoder_5_demos import plot_progress_class
...
def plot_matrix_as_image(matrix, names, set, text):
    ...
    fig, ax = plt.subplots(figsize=(len(matrix) * 1.25, len(matrix) * 1.25))
        ...
        for i, env_video_name in enumerate(tasks):
            ...
            for j, text_str in enumerate(text_list):
                ...
                if cache_key_full in cache_dict:
                    ...
                    triangular_mask = torch.zeros((128, 128))
                    for j in range(total_frames):
                        triangular_mask[j, :j+1] = 1
                    mask = torch.zeros(128)
                    mask[:total_frames] = 1
                    video_reward, _ = rewind_model(frames_embed.unsqueeze(0), triangular_mask.unsqueeze(1).unsqueeze(1), text_embed, mask)
...
def load_rewind_model(ckpt_path: str = "/scr/yusenluo/RoboCLIP/visualization/roboclip_v2_decoder_models_fix_3rd/RegressionFixTransformer5Demos_liv_sample_neg_heads_4_SampleNeg_ReverseVideo_norm_PositionalEncoding_500/model_199.pth") -> torch.nn.Module:
    # 1️⃣ 设定 `ckpt_path`
    embedding_dim = 1024
    ...
    # 3️⃣ 读取 `args`
    args = checkpoint["args"]  # 直接从 ckpt 里读取 args

    # 4️⃣ 计算 num_bins
    if args.catagorical_progress:
        num_bins = args.catagorical_progress_bins + 1 if args.sample_neg else args.catagorical_progress_bins
    else:
        num_bins = 1

    # 5️⃣ 选择模型（根据 args.two_step_training）
    ...
    # 6️⃣ 加载模型权重
    self_attention_model.load_state_dict(checkpoint["model"])
...
if __name__ == "__main__":
    ...
    wandb.init(project="roboclip-v2", name=f"eval_rewind")
    plot_progress_class(h5_eval_file, "train", rewind_model, args)
    plot_progress_class(h5_eval_file, "eval", rewind_model, args)
    plot_confusion_matrix_pca_class(h5_file = h5_eval_file, 
                            set = "train", 
                            self_attention_model = rewind_model, 
                            args = args)
    confusion_matrix = plot_confusion_matrix_pca_class(h5_file = h5_eval_file,
                                    set = "eval",
                                    self_attention_model = rewind_model,
                                    args = args)
    confusion_matrix_all_fail = plot_confusion_matrix_pca_class(h5_file = h5_eval_file_all_fail,
                                    set = "eval",
                                    self_attention_model = rewind_model,
                                    args = args)
    confusion_matrix_close_success = plot_confusion_matrix_pca_class(h5_file = h5_eval_file_close_success,
                                    set = "eval",
                                    self_attention_model = rewind_model,
                                    args = args)
