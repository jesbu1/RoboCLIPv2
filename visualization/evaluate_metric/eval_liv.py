...
def plot_matrix_as_image(matrix, names, set, text):
    ...
    wandb.log({f"confusion_matrix/{set}_confusion_matrix_LIV": wandb.Image(fig)})
    plt.savefig(f"confusion_matrix_{set}_LIV.pdf", bbox_inches="tight")
...
def generate_liv_data(
    ...
    # state_dict = torch.load('/scr/yusenluo/RoboCLIP/LIV/liv/train_liv_roboclip_19/2025-01-31_23-18-23/snapshot_10000.pt')["liv"]
    # liv.module.load_state_dict(state_dict)
    liv = liv.to(device)
                        ...
                        if total_frames < 1:
                            ...
                            for partial_count in range(1, total_frames + 1):
                                ...
                                if cache_key_partial in cache_dict:
                                    ...
                                    sub_frames_embed = embedding_image(liv, processor, Image.fromarray(frames_data[partial_count - 1].astype(np.uint8)))
...
if __name__ == "__main__":
    ...
    wandb.init(project="roboclip-v2", name=f"eval_LIV")
    ...
    confusion_matrix_all_fail, _, _, _ = generate_liv_data(
        h5_path="/scr/yusenluo/RoboCLIP/visualization/all_fail_videos.h5",
        json_path="new_task_v2.json",
        set_type="eval",
        device="cuda",
        cache_path="liv_cache_all_fail.pkl"
    )

    confusion_matrix_close_success, _, _, _ = generate_liv_data(
        h5_path="/scr/yusenluo/RoboCLIP/visualization/close_succ_videos.h5",
        json_path="new_task_v2.json",
        set_type="eval",
        device="cuda",
        cache_path="liv_cache_close_success.pkl"
    )
