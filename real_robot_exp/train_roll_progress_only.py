...
def main(args):
    ...
    if args.extra_data_type == "metaworld":
        experiment_name = "ProgressOnlyCrop_MetaWorld" 
    else: 
    ...
    for epoch in range(args.epochs):
        ...
        if args.openx_data:
            for openx_data, extra_data in tqdm(zip(openx_dataloader, extra_dataloader), total = 100):
                ...
                rest_openx_progress_loss = progress_loss_function(rest_openx_progress_pred[:,1:].squeeze(), rest_openx_progress_target[:,1:])
                rest_extra_progress_loss = progress_loss_function(rest_extra_progress_pred[:,1:].squeeze(), rest_extra_progress_target[:,1:])
