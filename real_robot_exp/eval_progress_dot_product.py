...
def plot_progress(h5_file, set, video_encoder, args, pca_text_model = None, pca_video_model = None):
    ...
    for key in tqdm(eval_envs):
        ...
        text_embeddings = text_embedding.repeat(1, video_embeddings.shape[0], 1).squeeze(0)

        if args.norm_length:
            video_embeddings = F.normalize(video_embeddings, p=2, dim=1)
            text_embeddings = F.normalize(text_embeddings, p=2, dim=1)
        progress = torch.sum(video_embeddings * text_embeddings, dim=-1).detach().cpu().numpy()
