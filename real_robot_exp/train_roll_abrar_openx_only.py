...
def main(args):
    ...
    if args.extra_data_type == "metaworld":
        experiment_name = "OpenX_Only_2_step_Crop_MetaWorld" 
    else: 
    ...
    openx_dataloader = DataLoader(openx_dataset, batch_size=openx_batch_size, shuffle=True, num_workers=8, drop_last=True, pin_memory=True)
    ...
    for epoch in range(args.epochs):
        ...
        for openx_data in tqdm(openx_dataloader):
            ...
            positive_video_array = torch.cat([openx_data["video_array"]], dim = 0).to(device).float()
            positive_text_array = torch.cat([openx_data["text_array"].squeeze(1)], dim = 0).to(device).float()              
            positive_progress = torch.cat([openx_data["progress"]], dim = 0).to(device).float()
            

            negative_video_array_1 = torch.roll(positive_video_array, 1, 0)[:int(openx_len * 0.25)]
            negative_text_array_1 = positive_text_array.clone()[:int(openx_len * 0.25)]
            negative_progress_1 = torch.zeros_like(positive_progress)[:int(openx_len * 0.25)]



            video_array = torch.cat([positive_video_array[:openx_len], negative_video_array_1[:openx_len]], dim = 0)
            text_array = torch.cat([positive_text_array[:openx_len], negative_text_array_1[:openx_len]], dim = 0)
            progress = torch.cat([positive_progress[:openx_len], negative_progress_1[:openx_len]], dim = 0)


            video_embedding = video_array

                # Binary classification targets
            openx_target = torch.cat([torch.ones(openx_len), torch.zeros(int(openx_len * 0.25))], dim=0).to(device)
            ...
            if args.two_step_training:
                openx_loss = focal_loss(class_pred, openx_target)

            
            valid_openx_progress_pred = progress_pred[openx_target.bool()]
            valid_openx_progress_target = progress[openx_target.bool()]
            ...
            if args.catagorical_progress:
                ...
                progress_loss = progress_loss_function(valid_openx_progress_pred[:,1:].squeeze(), valid_openx_progress_target[:,1:])

            loss = openx_loss  + progress_loss * args.progress_loss_weight
            ...
            wandb_log = {
                ...
                # OpenX metrics
                "train/openx_accuracy": compute_metrics(class_pred.squeeze(), openx_target)['accuracy'],
                "train/openx_precision": compute_metrics(class_pred.squeeze(), openx_target)['precision'],
                "train/openx_recall": compute_metrics(class_pred.squeeze(), openx_target)['recall'],
                "train/openx_f1": compute_metrics(class_pred.squeeze(), openx_target)['f1'],
            }
            wandb.log(wandb_log)
