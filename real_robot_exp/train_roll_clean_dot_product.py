    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            if extra_eval_eval_pos_dataset is not None:
                extra_pos_progress_loss = eval_model(extra_eval_eval_pos_dataloader, video_encoder, progress_loss_function, triangular_mask, args, 
                                                pca_text_model = pca_text_model, pca_video_model = pca_video_model, neg_sample=False)
                wandb_eval_log["extra_eval/paired_progress_loss"] = extra_pos_progress_loss
            
            if extra_eval_eval_neg_dataset is not None:
                extra_neg_progress_loss = eval_model(extra_eval_eval_neg_dataloader, video_encoder, progress_loss_function, triangular_mask, args, 
                                                pca_text_model = pca_text_model, pca_video_model = pca_video_model, neg_sample=True)
                wandb_eval_log["extra_eval/unpaired_progress_loss"] = extra_neg_progress_loss

            if positive_eval_openx_dataset is not None:
                openx_pos_progress_loss = eval_model(openx_positive_eval_dataloader, video_encoder, progress_loss_function, triangular_mask, args,
                                                                pca_text_model = pca_text_model, pca_video_model = pca_video_model, neg_sample=False)
                wandb_eval_log["openx_eval/paired_progress_loss"] = openx_pos_progress_loss
            
            if negative_eval_openx_dataset is not None:
                openx_neg_progress_loss = eval_model(openx_negative_eval_dataloader, video_encoder, progress_loss_function, triangular_mask, args,
                                                                pca_text_model = pca_text_model, pca_video_model = pca_video_model, neg_sample=True)
