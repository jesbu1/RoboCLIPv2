...
def main(args):
    ...
    experiment_name = "OpenXLIVMixDatasetAddNeg"
    ...
    batch_triangular_mask = triangular_mask.repeat(batch_size * 2 + args.batch_size, 1, 1, 1).bool()
    ...
    for epoch in range(args.epochs):
        ...
        if args.openx_data and args.extra_data:
            for openx_data, extra_data, negative_extra_data in tqdm(zip(openx_dataloader, extra_dataloader, negative_dataloader), total = 100):
                ...
                classification_loss_function, progress_loss_function, optimizer, openx_len = openx_len * 2, extra_len = extra_len * 3, scheduler = scheduler)
                wandb.log(wandb_log)
