...
def main(args):
    ...
    if args.openx_data and args.extra_data:
        ...
        extra_dataset =  LivVideoDecoderDataset5Frames(args, h5_file)
    ...
    for epoch in range(args.epochs):
        ...
        if args.openx_data and args.extra_data:
            for openx_data, extra_data in tqdm(zip(openx_dataloader, extra_dataloader), total = 100):
                ...
                text_array = torch.cat([openx_data["text_array"].squeeze(1), extra_data["text_array"]], dim = 0).to(device).float()
            ...
            for extra_data in tqdm(extra_dataloader):
                ...
                text_array = extra_data["text_array"].to(device).float()
