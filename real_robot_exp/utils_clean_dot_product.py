...
def eval_model_token(positive_eval_openx_dataset, video_encoder, text_encoder, progress_loss_function, triangular_mask, args, pca_text_model = None, pca_video_model = None, neg_sample = False):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    for eval_data in tqdm(positive_eval_openx_dataset):
        ...
        text_array = eval_data["text_array"].to(device).float()
        text_mask = eval_data["text_mask"].to(device).bool()
        progress = eval_data["progress"].to(device)
        progress_mask = torch.ones_like(progress).bool()
        if neg_sample:
            stop_idx = round(video_array.size(1) * args.negative_mask_ratio)
