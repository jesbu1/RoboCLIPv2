...
def plot_confusion_matrix(h5_file, model_name, set, self_attention_model):
...
def plot_confusion_matrix_pca(h5_file, model_name, set, self_attention_model, pca_text_model = None, pca_video_model = None, transform_model = None):
    device = next(self_attention_model.parameters()).device
    model, processor, tokenizer = load_model(model_name)
    if set == "train":
        eval_envs = json.load(open("task_subset.json"))["subset_6"]
...
def plot_confusion_matrix_pca_class(h5_file, model_name, set, self_attention_model, pca_text_model = None, pca_video_model = None, transform_model = None):
    device = next(self_attention_model.parameters()).device
    model, processor, tokenizer = load_model(model_name)
    if set == "train":
        eval_envs = json.load(open("task_subset.json"))["subset_6"]
    ...
    for i  in tqdm(range(len(eval_envs))):
        ...
        predicted_score, pred_class = self_attention_model(video_frame_data, mask=None, text_array=text_embeddings)
