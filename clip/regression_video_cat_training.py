...
from dataloader_clipliv_video import ClipLivVideoCatDataset
...
import h5py
from eval_video_attention_utils import plot_progress_fix, plot_progress_corr_fix, plot_videos_fix
...
def main(args):
    ...
    experiment_name = "RegressionRandomStartVideoWeightsLossSubstraction" + args.model_name + "_" + args.loss_type
    ...
    dataset = ClipLivVideoCatDataset(args, h5_file)
    ...
    if args.learn_params:
        # if args.model_name == "clip":
        #     context_parameters = torch.nn.Parameter(torch.randn(1,  args.sample_frames, device=device), requires_grad=True)
        #     # context_parameters.requires_grad = True
        #     # context_parameters = context_parameters.to(device)
        # elif args.model_name == "liv":
        context_parameters = torch.nn.Parameter(torch.randn(1,  args.sample_frames, 1, device=device), requires_grad=True)
            # context_parameters.requires_grad = True
            # context_parameters = context_parameters.to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for i, data in enumerate(tqdm(dataloader)):
            ...
            video_array = data["video_array"].to(device).float() # batch_size x num_frames x feature_dim
            
            # video
            weighted_array = video_array * context_parameters
            weighted_array = weighted_array.sum(dim=1) # batch_size x feature_dim


            if args.subtract:
                progress_input = weighted_array - text_array
            else:
                progress_input = torch.cat([text_array, weighted_array], dim=1)
        ...
        if epoch % 5 == 4:
            with torch.no_grad():
                corr_train_dict = plot_progress_corr_fix(h5_file, args.model_name, transform_model, "train", args.subtract, context_parameters)
                corr_eval_dict = plot_progress_corr_fix(h5_file, args.model_name, transform_model, "eval", args.subtract, context_parameters)
                wandb.log(corr_train_dict)
                wandb.log(corr_eval_dict)
            
                plot_progress_fix(h5_file, args.model_name, transform_model, "train", args.subtract, context_parameters)
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--sample_frames', type=int, default=15)
