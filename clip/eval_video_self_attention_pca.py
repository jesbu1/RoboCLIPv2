...
def plot_progress(h5_file, model_name, set, self_attention_model, text_pca = None, video_pca = None, transformation_model = None):
    ...
    if text_pca is not None:
        wrong_text_embedding = text_pca.transform(wrong_text_embedding.detach().cpu().numpy())
        wrong_text_embedding = torch.tensor(wrong_text_embedding).to(device).float()
    ...
    for i  in tqdm(range(len(eval_envs))):
        ...
        for i in range(traj_data.shape[0]):
            ...
            if video_pca is not None:
                video_frame_data = video_frame_data.view(-1, 1024)
                video_frame_data = video_pca.transform(video_frame_data.cpu().detach().numpy())
                video_frame_data = torch.tensor(video_frame_data).to(device).float()
                video_frame_data = transformation_model(video_frame_data)
                video_frame_data = video_frame_data.view(1, -1, video_frame_data.shape[1])
