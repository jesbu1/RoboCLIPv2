...
class ClipLivRandomStartDataset(ClipLivSingleDataset):
    ...
    def __getitem__(self, idx):
        ...
        text_array = self.sample_text_feature(key)
        # sample video sample
        start_frame, choose_frame, progress = self.sample_random_start_end_feature(key)
    ...
    def sample_random_start_end_feature(self, env_name):
        progress_group = self.h5_file[self.model_name][env_name]
        # choose index
        datasets = sorted(list(progress_group.keys()), key = int)[:15]
        random_name = random.choice(datasets)
        ...
        sample_idx = random.randint(start_idx+1, len(progress_dataset)-1)

        start_frame = np.asarray(progress_dataset[start_idx])
        choose_frame = np.asarray(progress_dataset[sample_idx])

        length = len(progress_dataset) - 1 - start_idx
        progress = (sample_idx - start_idx + 1) / length


        return start_frame, choose_frame, progress
