...
def plot_progress_corr(h5_file, model_name, transform_model, set, text_pca_model, image_pca_model, linear_model):
    device = next(transform_model.parameters()).device
    model, processor, tokenizer = load_model(model_name)
    if set == "train":
        eval_envs = json.load(open("task_subset.json"))["subset_6"]
...
def plot_progress(h5_file, model_name, transform_model, set, text_pca_model, image_pca_model, linear_model):
    device = next(transform_model.parameters()).device
    model, processor, tokenizer = load_model(model_name)
    if set == "train":
        eval_envs = json.load(open("task_subset.json"))["subset_6"]
...
def plot_videos(model_name, transform_model, text_pca_model, image_pca_model, linear_model):
    device = next(transform_model.parameters()).device
    model, processor, tokenizer = load_model(model_name)
