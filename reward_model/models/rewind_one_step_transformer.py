...
class RewardTwoStepNewPositionEmbeddingPredictor(nn.Module):
    def __init__(self, input_dim, args, class_num):
        ...
        self.transformer_decoder = nn.ModuleList(
            [
                DecoderOnlyBlock(
                    input_dim, args.attention_heads, input_dim, args.layer_norm
                )
        ...
        if args.positional_encoding:
            self.position_embedding = self._get_cosine_positional_encoding(
                args.max_length, input_dim
            )
...
class ClassProgressTransformer(nn.Module):
    def __init__(
        ...
        if self.args.positional_encoding:
            self.first_pos_embed = nn.Parameter(
        ...
        self.attention_mask = nn.Transformer.generate_square_subsequent_mask(18).to(
            "cuda"
        )
    ...
    def forward(self, video_frames, text_embed, attention_mask=None):
        ...
        text_embed = self.text_proj(text_embed).unsqueeze(
            1
        ...
        if attention_mask is not None:
            ...
            extended_mask = torch.ones(
                (batch_size, 2), device=attention_mask.device
            )  # class token + text token
            attention_mask = torch.cat([extended_mask, attention_mask], dim=1)

        # Pass through transformer

        transformed = self.transformer(
            sequence, is_causal=True, mask=self.attention_mask
        )
        ...
        progress_preds = self.progress_head(
            transformed[:, 1:-1]
        )  # Exclude class token and text token
