...
class BaseRewardModel(abc.ABC):
    ...
    def encode_images(self, images: np.ndarray) -> np.ndarray:
        ...
        if images.shape[-1] == 3 and not images.shape[2] == 3:
            ...
            # print("shape after transpose", images.shape) # (num_vids, num_frames, 3, H, W) (1,128,3,480,640)
        for i in range(0, len(images), self.batch_size):
            batch_images = images[i : i + self.batch_size]
            batch_images = torch.tensor(batch_images, dtype=torch.float32).to(
                self.device
            )
            # print("batch_images shape", batch_images.shape) # (1,128,3,480,640)
    ...
    def calculate_rewards(
        ...
        """
        # print(encoded_texts.shape, encoded_videos.shape) #(1, 384) (1, 32, 768)
