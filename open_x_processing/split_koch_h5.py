...
h5_file_name = "usc_koch_rewind_dino_reward_main.h5"
...
train_file = h5py.File("usc_koch_rewind_dino_reward_main_train.h5", 'w')
for key in train_keys:
    h5_file.copy(key, train_file)
print(len(train_file.keys()))
train_file.close()

eval_file = h5py.File("usc_koch_rewind_dino_reward_main_eval.h5", 'w')
for key in Eval_keys:
    h5_file.copy(key, eval_file)
print(len(eval_file.keys()))
eval_file.close()
