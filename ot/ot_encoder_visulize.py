'''
This file is used to test the encoder model.
Use S3D encoder, then visualize the output from sthsth dataset and metaworld videos.
Use PCA to reduce the dimension to 2D, then plot the 2D points.
See the distribution of the videos from two different datasets.
'''
...
if __name__ == "__main__":
    csv_file_path = "SubspaceAlignment_Result_Sth.csv"
    ...
    s3d.eval()
    plot_dir_name = 'Sth_Train'
    if args.validation:
        plot_dir_name = 'Sth_Validate'
        ...
        validate_data_loader = DataLoader(
            validate_dataset, batch_size=50, shuffle=False, num_workers=5
        )
        (
    ...
    sample_sizes = np.array([4, 8, 21]) * START_SAMPLE_SIZE
    for sample_size in sample_sizes:
        ...
        simi_score = th.mean(
            th.diag(th.matmul(train_video_embeddings_normalized, train_text_embeddings_normalized.t())))
        ...
        for variance_threshold in variance_thresholds:
            ...
            simi_score_model = th.mean(
                th.diag(
                    th.matmul(adjusted_video_embeddings.cpu().float(), th.from_numpy(text_embeddings_pca).float().t())))
            data_to_write_model= [
