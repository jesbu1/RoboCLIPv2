#!/usr/bin/env bash
# Submit raw no-chat TOPReward baseline+diff gamma1 seed-configurable pipeline:
...
read -r -a GROUP_IDS <<< "$GROUP_IDS_RAW"
GROUP_LABELS=(b0 b1 b2 b3 d0 d1 d2 d3)
SERVER_JOB_IDS=()
SERVER_INFO_FILES=()

if [ "${#GROUP_IDS[@]}" -ne 8 ]; then
  echo "ERROR: GROUP_IDS_RAW must contain exactly 8 server group ids, got ${#GROUP_IDS[@]}: ${GROUP_IDS_RAW}"
  exit 1
fi

BASELINE_H5="${TOPREWARD_DIR}/datasets/metaworld_topreward_raw_baseline_seed${SEED}_${RUN_TAG}.h5"
DIFF_H5="${TOPREWARD_DIR}/datasets/metaworld_topreward_raw_diff_gamma1_H${HORIZON}_seed${SEED}_${RUN_TAG}.h5"
BASELINE_OFFLINE_DIR="logs/offline_topreward_raw_baseline_H${HORIZON}_10critics_seed${SEED}_${RUN_TAG}"
DIFF_OFFLINE_DIR="logs/offline_topreward_raw_diff_gamma1_scale_hplus1_over2_H${HORIZON}_10critics_seed${SEED}_${RUN_TAG}"
MANIFEST="${PROJECT_DIR}/logs/topreward_raw_baseline_diff_seed${SEED}_8servers_${RUN_TAG}.txt"

{
  ...
  echo "server_group_ids=$(IFS='|'; echo "${GROUP_IDS[*]}")"
...
for idx in "${!GROUP_IDS[@]}"; do
  ...
  info_file="${PROJECT_DIR}/logs/topreward_raw_server_seed${SEED}_group${group_id}_${RUN_TAG}_info.txt"
  rm -f "$info_file" "${info_file}.tmp"

  server_job_id=$(sbatch --parsable \
    --job-name="top_raw_${JOB_SEED_TAG}_${label}" \
    --account=biyik_1165 \
    --partition=gpu \
    --gres=gpu:1 \
    ...
    --mem="$SERVER_MEM" \
    --time="$SERVER_TIME" \
    --output="logs/topreward_raw_server_seed${SEED}_group${group_id}_${RUN_TAG}_%j.out" \
    --error="logs/topreward_raw_server_seed${SEED}_group${group_id}_${RUN_TAG}_%j.err" \
    --export=ALL,PROJECT_DIR="$PROJECT_DIR",TOPREWARD_DIR="$TOPREWARD_DIR",GROUP_ID="$group_id",SERVER_LABEL="topreward_raw_seed${SEED}_group_${group_id}",INFO_FILE="$info_file",SERVER_BACKEND="$SERVER_BACKEND",TOPREWARD_ATTN_IMPLEMENTATION="$TOPREWARD_ATTN_IMPLEMENTATION" \
...
launcher_job_id=$(sbatch --parsable \
  ...
  --output="logs/top_lau_raw_${JOB_SEED_TAG}_%j.out" \
  --error="logs/top_lau_raw_${JOB_SEED_TAG}_%j.err" \
  --export=ALL,PROJECT_DIR="$PROJECT_DIR",TOPREWARD_DIR="$TOPREWARD_DIR",SERVER_JOB_IDS="$SERVER_JOB_IDS_JOINED",SERVER_INFO_FILES="$SERVER_INFO_FILES_JOINED",SERVER_GROUP_IDS="$SERVER_GROUP_IDS_JOINED",SUBMIT_MANIFEST="$MANIFEST",RUN_TAG="$RUN_TAG",SEED="$SEED",HORIZON="$HORIZON",MAX_FRAMES="$MAX_FRAMES",REQUEST_TIMEOUT="$REQUEST_TIMEOUT",REQUEST_RETRIES="$REQUEST_RETRIES",TOPREWARD_REQUEST_FORMAT="$TOPREWARD_REQUEST_FORMAT",BASELINE_H5="$BASELINE_H5",DIFF_H5="$DIFF_H5",BASELINE_OFFLINE_DIR="$BASELINE_OFFLINE_DIR",DIFF_OFFLINE_DIR="$DIFF_OFFLINE_DIR" \
