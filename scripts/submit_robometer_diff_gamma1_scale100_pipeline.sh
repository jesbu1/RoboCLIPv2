#!/usr/bin/env bash
# Submit Robometer diff-gamma=1.0 scale=100 pipeline:
# 8 servers -> first-ready label -> 3-seed offline -> 8 online groups, 3 seeds each.
...
MANIFEST="${PROJECT_DIR}/logs/robometer_diff_gamma1_scale100_pipeline_$(date +%Y%m%d_%H%M%S).txt"
SERVER_JOB_IDS=()
SERVER_INFO_FILES=()
...
for idx in "${!GROUP_IDS[@]}"; do
  ...
  env_id="${ENV_IDS[$idx]}"
  info_file="${PROJECT_DIR}/logs/robometer_server_gamma1_scale100_group${group_id}_info.txt"
  rm -f "$info_file" "${info_file}.tmp"

  server_job_id=$(sbatch --parsable \
    --job-name="robo_srv_d1_g${group_id}" \
    --output="logs/robo_srv_d1_g${group_id}_%j.out" \
    --error="logs/robo_srv_d1_g${group_id}_%j.err" \
    --export=ALL,PROJECT_DIR="${PROJECT_DIR}",ROBOMETER_PROJECT_DIR="${ROBOMETER_PROJECT_DIR}",GROUP_ID="${group_id}",SERVER_LABEL="diff gamma1 scale100 ${env_id}",INFO_FILE="${info_file}",SERVER_PORT="${SERVER_PORTS[$idx]}",FALLBACK_START="${FALLBACK_STARTS[$idx]}",FALLBACK_END="${FALLBACK_ENDS[$idx]}",STARTUP_DELAY_SEC="${STARTUP_DELAYS[$idx]}",STARTUP_MAX_ATTEMPTS=6,STARTUP_BACKOFF_BASE_SEC=120 \
...
launcher_id=$(sbatch --parsable \
  --job-name="robo_launch_d1_s100" \
  --output="logs/robo_launch_d1_s100_%j.out" \
  --error="logs/robo_launch_d1_s100_%j.err" \
  --export=ALL,PROJECT_DIR="$PROJECT_DIR",SERVER_JOB_IDS="$SERVER_JOB_IDS_JOINED",SERVER_INFO_FILES="$SERVER_INFO_FILES_JOINED",SUBMIT_MANIFEST="$MANIFEST",MAX_FRAMES="$MAX_FRAMES" \
  scripts/robometer_launch_diff_gamma1_scale100_pipeline.sbatch)
