...
# Uses 9 existing server jobs in group40..48 order. No Slurm arrays.
...
MANIFEST="${PROJECT_DIR}/logs/robometer_selected_diff_gamma1_scale_hplus1_over2_H${HORIZON}_9tasks_3reruns_existing_9servers_$(date +%Y%m%d_%H%M%S).txt"
...
for idx in "${!TASK_SPECS[@]}"; do
  ...
  for rep in $(seq 1 "$REPEAT_COUNT"); do
    ...
    echo "online_${env_id}_seed${seed}_rep${rep}=${online_id} depends_on=${server_job_id}" | tee -a "$MANIFEST"
  done
done
