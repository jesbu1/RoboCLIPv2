            ...
            for i in range(len(text_embedding_all)):
                ...
                for j in range(len(states)):
                    ...
                    if args.reward_type == "dense":
                        pre_training_h5_file["rewards"][current_step] = rewards[j]
                    elif args.reward_type == "sparse":
                        pre_training_h5_file["rewards"][current_step] = np.float32(done[j])
                    elif args.reward_type == "rewind":
                        #if done[j] == 1:
                        #    model_input_image_embedding = subsample_video(traj_img_embeddings, max_length=16).unsqueeze(0).to(device)
                        #    model_input_text_embedding = torch.tensor(text_embedding).unsqueeze(0).to(device)
                        #    reward_seq, _ = model(model_input_image_embedding, model_input_text_embedding)
                        #    # reward_sum = torch.sum(reward_seq) / args.divisor + SUCCESS_BONUS
                        #    reward_sum = torch.sum(reward_seq) / args.divisor
                        #    pre_training_h5_file["rewards"][current_step] = reward_sum.item()
                            
                        #else:
                        #    pre_training_h5_file["rewards"][current_step] = 0
                        input_video_frames = traj_img_embeddings[:j + 1]
                        input_video_frames = subsample_video(input_video_frames, 16).unsqueeze(0).to(device)
